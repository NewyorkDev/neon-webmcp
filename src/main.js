...
function customerView(state) {
  ...
  return `<main>
    <section class="hero">
      ...
      <p class="hero-lede">${t(state, 'Ask your assistant to find or rebook the right appointment. Every decision stays visible.', 'Pide a tu asistente que encuentre o vuelva a reservar la cita correcta. Cada decisión permanece visible.')}</p>
...
function customerAccountView(state) {
  ...
  return `<main class="account-main page-width">
    ...
    <section class="account-grid">
      ...
      <article class="account-card"><p class="eyebrow dark">${t(state, 'ACCOUNT DETAILS', 'DATOS DE LA CUENTA')}</p><dl class="account-details"><div><dt>${t(state, 'Email', 'Correo')}</dt><dd>${customer.email}</dd></div><div><dt>${t(state, 'Home area', 'Zona')}</dt><dd>${customer.home}</dd></div><div><dt>${t(state, 'Interface', 'Interfaz')}</dt><dd>${state.locale === 'en' ? 'English' : 'Español'}</dd></div><div><dt>${t(state, 'Payment', 'Pago')}</dt><dd>${t(state, 'No card required for this demo', 'No se requiere tarjeta')}</dd></div></dl></article>
    </section>
...
function header(state) {
  return `<header class="site-header"><button class="brand" id="home-button"><img src="./images/booksy-reloaded-logo.png" alt=""><span><b>BOOKSY</b><small>RELOADED</small></span></button><nav><button data-role="customer" class="${state.role === 'customer' && customerPage === 'marketplace' ? 'active' : ''}">${t(state, 'Explore', 'Explorar')}</button><button id="customer-account-nav" class="${state.role === 'customer' && customerPage === 'account' ? 'active' : ''}">${t(state, 'My appointments', 'Mis citas')}</button><button data-role="provider" class="${state.role === 'provider' ? 'active' : ''}">${t(state, 'Provider workspace', 'Área profesional')}</button></nav><div class="header-actions"><button id="locale-button">${icon('globe')} ${state.locale === 'en' ? 'ES' : 'EN'}</button><button id="account-button" class="account-button">${icon('user')}<span><small>${state.role === 'customer' ? (customerLoggedIn ? t(state, 'SIGNED IN', 'SESIÓN ACTIVA') : t(state, 'CUSTOMER LOGIN', 'INICIAR SESIÓN')) : t(state, 'PROVIDER DEMO', 'PROFESIONAL DEMO')}</small><b>${state.role === 'customer' && !customerLoggedIn ? t(state, 'Demo account', 'Cuenta demo') : state.role === 'customer' ? 'Alex Morgan' : 'Marco Ruiz'}</b></span></button></div></header>`;
}
